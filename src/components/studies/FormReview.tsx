...
const FormReview = () => {
  ...
  const demoQuestions = [
    {
      ...
      required: true,
      media: [{type: 'image', url: '/lovable-uploads/2f07911a-86af-4ccd-a91d-84bf490a65c5.png'}]
    },
    {
      id: 2,
      type: 'open-ended',
  ...
  const renderResponseOptions = (question: any) => {
    ...
    return (
      <div className="mt-3 p-3 bg-gradient-to-r from-blue-50 to-orange-50 rounded-lg border border-blue-200">
        <div className="text-xs font-medium text-gray-600 mb-2">Response Options:</div>
        <div className="flex gap-2">
          {responseTypes.map((type: string) => (
            <div key={type} className="flex items-center gap-1 px-2 py-1 bg-white rounded border">
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <div className="flex gap-2">
          ...
          <Button className="flex items-center gap-2 bg-gradient-to-r from-blue-500 to-orange-500 hover:from-blue-600 hover:to-orange-600 text-white">
        ...
        <CardContent className="space-y-6 p-6">
          ...
          <div className="space-y-6">
            ...
            {demoQuestions.map((question, index) => (
              <Card key={question.id} className="border-l-4 border-l-gradient shadow-md hover:shadow-lg transition-shadow bg-white">
                <CardContent className="p-6">
                  <div className="flex items-start justify-between mb-4">
                    <div className="flex items-center gap-3">
                      ...
                      <div className="flex flex-wrap gap-2">
                        ...
                        {question.media && question.media.map((media, idx) => (
                    ...
                    <div className="mb-4 flex gap-3">
                      {question.media.map((media, idx) => (
                        <div key={idx} className="relative">
                          {media.type === 'image' && (
                            <div className="w-20 h-20 rounded-lg overflow-hidden bg-gray-100 border-2 border-blue-200">
                              <img src={media.url} alt="Question media" className="w-full h-full object-cover" />
                            </div>
                          )}
                          {media.type === 'video' && (
                            <div className="w-20 h-20 rounded-lg bg-green-100 border-2 border-green-200 flex items-center justify-center">
                              <Play className="h-8 w-8 text-green-600" />
                            </div>
                          )}
                          {media.type === 'webpage' && (
                            <div className="w-20 h-20 rounded-lg bg-orange-100 border-2 border-orange-200 flex items-center justify-center">
                  ...
                  {question.type === 'multiple-choice' && question.options && (
                    <div className="space-y-3">
                      {question.options.map((option, optIndex) => (
                        <label key={optIndex} className="flex items-center gap-3 p-3 border border-gray-200 rounded-lg hover:bg-blue-50 hover:border-blue-300 cursor-pointer transition-all">
                  ...
                  {question.type === 'open-ended' && (
                    <div className="border-2 border-gray-200 rounded-lg p-4 bg-gray-50 min-h-[100px] relative">
                  ...
                  {question.type === 'voice-response' && (
                    <div className="border-2 border-orange-200 rounded-lg p-4 bg-orange-50 min-h-[100px] relative">
                    ...
                    <div className="space-y-3">
                      ...
                      <div className="flex gap-2">
                        {[...Array(10)].map((_, i) => (
                          <button 
                            key={i} 
                            className="w-10 h-10 border-2 border-purple-300 rounded-lg flex items-center justify-center text-sm font-medium hover:bg-purple-100 hover:border-purple-400 transition-all"
                          >
                            {i + 1}
                          </button>
                        ))}
